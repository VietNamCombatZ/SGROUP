const app = {
  ...
  async getMostExpensiveProduct() {
    ...
    return `
            ...
            <p>Price: ${data.currency.longFormat.replace(
              "{0}",
              maxVariant.price
            )}</p>
            <div>Option:</div>
  ...
  async getTotalVariantPrice() {
    ...
    const product = data.items.find(
      (product) => product.title === "Test bundle"
    );
    if (product) {
      let totalPrice = product.variants.reduce(
      ...
      return `
                <h2>${product.title}</h2>
                <p>${product.variants.length} variant${
  ...
  async getSoldOutProduct() {
    const data = await app.fetchData();
    const soldOutProducts = data.items.filter((product) =>
      product.variants.every((variant) => variant.available === 0)
    );
  ...
  async getProductWithOption(option, value) {
    ...
    const products = data.items.filter((product) => {
      const hasOption = product.options.find(
        (opt) => opt.name.toLowerCase() === option
      );
      if (!hasOption) return false;
      const hasValue = hasOption.values.find(
    ...
    return products
      .map((product) => {
        ...
        const valueIndex = product.options[optionIndex].values.findIndex(
          (val) => val.toLowerCase() === value
        );
        ...
        variants = variants
          .map((variant) => {
            let option = variant.options
              .map((val, index) => {
                if (index === optionIndex) return "";
                return (
                  productOptions[index].name +
                  ": " +
                  productOptions[index].values[val]
                );
              })
              .join("");

            return `<div>${option} | ${data.currency.longFormat.replace(
              "{0}",
              variant.price
            )}</div>`;
          })
  ...
  bai4() {
    ...
    const result = document.querySelector(".result");
    button.addEventListener("click", async function () {
      result.innerHTML = await app.getProductWithOption(
        optionInput.value,
  ...
  bai5() {
    ...
    const result = document.querySelector(".result");
    button.addEventListener("click", async function () {
      result.innerHTML = await app.getProductWithPrice(
        minPrice.value,
        maxPrice.value
      );
    });
